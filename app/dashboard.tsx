...
import {
  Activity, ArrowLeft, ArrowRight, Box, Cable, Check, CircleAlert, Copy,
  Download, ExternalLink, FileKey2, FolderOpen, LayoutDashboard, Menu, Plus,
  RefreshCw, Settings2, Upload, X,
} from "lucide-react";
...
function apiRequestUrl(apiBase: string, path: string): string {
...
function EmptyState({ title, body, action }: { title: string; body: string; action?: ReactNode }) {
  return <div className="empty-state"><svg className="empty-mark" width="34" height="34" viewBox="0 0 32 32" aria-hidden="true"><rect width="32" height="32" rx="8" fill="#171717" /><path d="M8 10.5h16l-1.6 12.2a3 3 0 0 1-3 2.6h-6.8a3 3 0 0 1-3-2.6L8 10.5Z" fill="#fff" /><path d="M7 8.5A1.5 1.5 0 0 1 8.5 7h15a1.5 1.5 0 0 1 0 3h-15A1.5 1.5 0 0 1 7 8.5Z" fill="#fff" /><path d="M12 15h8M12.7 19h6.6" stroke="#171717" strokeWidth="2" strokeLinecap="round" /></svg><h3>{title}</h3><p>{body}</p>{action}</div>;
}
function Modal({ title, description, children, onClose }: { title: string; description?: string; children: ReactNode; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal-card" role="dialog" aria-modal="true" aria-labelledby="modal-title"><div className="modal-head"><div><p className="eyebrow">OpenBucket</p><h2 id="modal-title">{title}</h2>{description ? <p>{description}</p> : null}</div><button ref={closeRef} className="icon-button" type="button" onClick={onClose} aria-label="Close dialog"><X size={16} /></button></div>{children}</section></div>;
}
function CopyButton({ value, label = "Copy" }: { value: string; label?: string }) {
  const [copied, setCopied] = useState(false);
  return <button className="copy-button" type="button" onClick={async () => { await navigator.clipboard.writeText(value); setCopied(true); window.setTimeout(() => setCopied(false), 1400); }}>{copied ? <><Check size={13} /> Copied</> : <><Copy size={13} /> {label}</>}</button>;
}
...
export function Dashboard({ initialConnection }: { initialConnection?: { apiBase: string; token: string; displayUrl?: string } } = {}) {
  ...
  const navItems: Array<{ id: View; label: string; icon: typeof LayoutDashboard }> = [
    { id: "overview", label: "Overview", icon: LayoutDashboard }, { id: "buckets", label: "Buckets", icon: Box }, { id: "keys", label: "API keys", icon: FileKey2 }, { id: "connections", label: "Connections", icon: Cable }, { id: "logs", label: "Logs & analytics", icon: Activity },
  ];
  ...
  return <div className="app-shell"><a className="skip-link" href="#main-content">Skip to content</a><aside className={`sidebar ${mobileNavOpen ? "open" : ""}`}><div className="brand-row"><svg className="brand-mark" width="30" height="30" viewBox="0 0 32 32" aria-hidden="true"><rect width="32" height="32" rx="8" fill="#171717" /><path d="M8 10.5h16l-1.6 12.2a3 3 0 0 1-3 2.6h-6.8a3 3 0 0 1-3-2.6L8 10.5Z" fill="#fff" /><path d="M7 8.5A1.5 1.5 0 0 1 8.5 7h15a1.5 1.5 0 0 1 0 3h-15A1.5 1.5 0 0 1 7 8.5Z" fill="#fff" /><path d="M12 15h8M12.7 19h6.6" stroke="#171717" strokeWidth="2" strokeLinecap="round" /></svg><div><strong>OpenBucket</strong><span>Node console</span></div><button className="mobile-close icon-button" type="button" aria-label="Close navigation" onClick={() => setMobileNavOpen(false)}><X size={16} /></button></div><nav aria-label="Node dashboard">{navItems.map((item) => { const Icon = item.icon; return <button key={item.id} className={activeView === item.id ? "active" : ""} type="button" onClick={() => { setActiveView(item.id); setMobileNavOpen(false); }}><Icon size={17} aria-hidden="true" />{item.label}</button>; })}</nav><div className="sidebar-node"><div className="node-state"><span className={`status-dot ${loadState === "connected" ? "online" : "offline"}`} /><span>{loadState === "connected" ? "Daemon online" : "Daemon offline"}</span></div><strong>{status?.nodeName ?? "No node connected"}</strong><small>{status?.storageRoot ?? "OpenBucket node"}</small><button type="button" onClick={() => setConnectionOpen(true)}><Settings2 size={13} /> Connection settings</button></div></aside>{mobileNavOpen ? <button className="sidebar-scrim" type="button" aria-label="Close navigation" onClick={() => setMobileNavOpen(false)} /> : null}<div className="workspace"><header className="topbar"><button className="mobile-menu icon-button" type="button" aria-label="Open navigation" onClick={() => setMobileNavOpen(true)}><Menu size={18} /></button><div className="breadcrumbs"><span>OpenBucket</span><b>/</b><strong>{navItems.find((item) => item.id === activeView)?.label}</strong></div><div className="topbar-actions"><span className="last-updated">{lastUpdated ? `Updated ${lastUpdated.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Not connected"}</span><button className="icon-button" type="button" aria-label="Refresh data" onClick={() => void refresh()}><RefreshCw size={15} /></button><a className="docs-link" href={docsUrl} target="_blank" rel="noreferrer">Docs <ExternalLink size={13} /></a></div></header><main id="main-content" className="main-content">{activeView === "overview" ? renderOverview() : null}{activeView === "buckets" ? renderBuckets() : null}{activeView === "keys" ? renderKeys() : null}{activeView === "connections" ? renderConnections() : null}{activeView === "logs" ? renderLogs() : null}</main></div>
    ...
    {connectionOpen ? <ConnectionModal apiBase={apiBase} adminToken={adminToken} onSave={saveConnection} onClose={() => setConnectionOpen(false)} /> : null}<div className="toast-region" aria-live="polite">{toasts.map((toast) => <div className={`toast ${toast.tone}`} key={toast.id}><span>{toast.tone === "success" ? <Check size={13} /> : <CircleAlert size={13} />}</span>{toast.message}</div>)}</div></div>;
}
