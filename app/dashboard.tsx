...
export function Dashboard() {
  ...
  const renderConnections = () => <section><div className="page-heading"><div><p className="eyebrow">Developer setup</p><h1>Connections</h1><p>Use standard S3 clients. Change the endpoint, keep the ecosystem.</p></div><button className="button secondary compact" type="button" onClick={() => setConnectionOpen(true)}>Dashboard connection</button></div><div className="endpoint-grid">{[
      ["Dashboard API", apiBase, "The authenticated management origin used by this browser"], ["Local S3 endpoint", clientConfig.s3Url ?? status?.endpoints.s3 ?? "Not connected", "AWS Signature V4 object API on the storage host"], ["Public S3 endpoint", clientConfig.publicBaseUrl ?? status?.endpoints.public ?? "Not configured", "Optional tunnel or reverse proxy"], ["File sharing", clientConfig.filesUrl ?? status?.endpoints.files ?? "Not configured", "Expiring browser-friendly links"],
    ].map(([label, value, note]) => <article className="endpoint-card" key={label}><p className="eyebrow">{label}</p><div className="endpoint-value"><code>{value}</code>{!value.startsWith("Not ") ? <CopyButton value={value} /> : null}</div><p>{note}</p></article>)}</div><article className="code-panel"><div className="code-panel-head"><div><p className="eyebrow">Copy, paste, connect</p><h2>Client configuration</h2></div><CopyButton value={snippets[snippetTab]} label="Copy snippet" /></div><div className="tabs" role="tablist" aria-label="Client examples">{(["javascript", "python", "aws", "curl"] as const).map((tab) => <button key={tab} role="tab" aria-selected={snippetTab === tab} type="button" onClick={() => setSnippetTab(tab)}>{tab === "aws" ? "AWS CLI" : tab[0].toUpperCase() + tab.slice(1)}</button>)}</div><pre><code>{snippets[snippetTab]}</code></pre></article><article className="env-panel"><div><p className="eyebrow">Environment</p><h2>Everything is configurable.</h2><p>Keep brand URLs, listen addresses, ports, origins, storage paths, and dashboard URLs outside source code.</p></div><div className="env-list"><code>OPENBUCKET_S3_PORT=8333</code><code>OPENBUCKET_MANAGEMENT_PORT=7272</code><code>OPENBUCKET_PUBLIC_BASE_URL=https://s3-node.openbucket.dev</code><code>NEXT_PUBLIC_OPENBUCKET_API_URL=http://127.0.0.1:7272</code></div></article></section>;
